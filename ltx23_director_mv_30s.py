...
- This notebook is **T4 GPU compatible** with optimized defaults (832x480, 97 frames,
  batch processing, skip_pass2). For A100/L4, you can increase resolution and enable
  the full two-pass pipeline. The 22B UNet model uses GGUF Q4_K_M quantization for
...
# @markdown **T4 GPU compatible with optimized batch frame generation and memory management.**
...
def upload_multiple_images(count=5):
    ...
    print(f"Upload up to {count} reference image(s) for timeline segments:")
    uploaded = files.upload()
...
def get_t4_optimal_settings():
    """Return recommended settings for T4 GPU (16GB VRAM).

    These defaults are optimized to fit the LTX-2.3 Director pipeline within
    T4 memory constraints by using lower resolution, fewer frames, batch
    processing, and skipping the high-res refine pass.
    """
    settings = {
        "batch_frames": 73,
        "width": 832,
...
# @markdown Upload up to 5 reference images for timeline segments.
upload_images_now = True  # @param {type:"boolean"}
image_files = []
if upload_images_now:
    image_files = upload_multiple_images(count=5)
    clear_output()
    print(f"Uploaded {len(image_files)} image(s) for timeline segments.")
...
def mainLTXDirector(
    ...
    with torch.inference_mode():
        ...
        # Free LoRA loader to reclaim memory
        del loraloader
        torch.cuda.synchronize()
        torch.cuda.empty_cache()
        gc.collect()
        ...
        if not skip_pass2:
            ...
            # Set decode sources from Pass 2 output
            video_latent_for_decode = get_value_at_index(ltxdirectorcropguides_54, 2)
            audio_latent_for_decode = get_value_at_index(ltxvseparateavlatent_22, 1)

        else:
            # skip_pass2=True: Decode directly from Pass 1 output (saves VRAM on T4)
            print("Skipping Pass 2 (T4 memory optimization)...")
            del unetloadergguf_135
            torch.cuda.synchronize()
            torch.cuda.empty_cache()
            gc.collect()

            video_latent_for_decode = get_value_at_index(ltxvseparateavlatent_34, 0)
            audio_latent_for_decode = get_value_at_index(ltxvseparateavlatent_34, 1)
        ...
        vaedecode_result = vaedecode.decode(
            samples=video_latent_for_decode,
            vae=get_value_at_index(vaeloader_36, 0),
        )
        ...
        ltxvaudiovaedecode_result = ltxvaudiovaedecode.EXECUTE_NORMALIZED(
            samples=audio_latent_for_decode,
            audio_vae=get_value_at_index(vaeloader_8, 0),
        )
...
# @markdown **T4 GPU optimized. Increase resolution and disable skip_pass2 on A100/L4.**

# @markdown ### Main Settings (T4-optimized defaults)
...
# @markdown ### T4 Memory Optimization
batch_frames = 73  # @param {"type":"integer"}
skip_pass2 = True  # @param {type:"boolean"}
